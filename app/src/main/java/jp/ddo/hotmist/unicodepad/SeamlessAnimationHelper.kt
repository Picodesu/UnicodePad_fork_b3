...
class SeamlessAnimationHelper(private val context: Context) {
    ...
    fun skipReturnAnimation(activity: Activity) {
        try { OplusViewSeamless.skipBackAnim(activity) } catch (e: Exception) { Log.e(TAG, "skipReturnAnimation error", e) }
    }

    fun setSkipReturnSeamless(activity: Activity) {
        try { OplusViewSeamless.setSkipViewSeamless(activity) } catch (e: Exception) { Log.e(TAG, "setSkipReturnSeamless error", e) }
    }

    fun setForceAlphaFadeOut(activity: Activity, force: Boolean) {
        try { OplusViewSeamless.setForceLeashAlphaOut(activity, force) } catch (e: Exception) { Log.e(TAG, "setForceAlphaFadeOut error", e) }
    }

    fun stopCurrentAnimation(): Boolean {
        return try { OplusViewSeamless.finishCurrentAnimation() } catch (e: Exception) { Log.e(TAG, "stopCurrentAnimation error", e); false }
    }

    fun getVersion(): Int {
        return try { OplusViewSeamless.getVersion() } catch (e: Exception) { -1 }
    }
}
